...
func TestNewMWei(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "100 wei",
			args: args{value: big.NewFloat(0.0001)},
			want: &Unit{Value: big.NewInt(100)},
		},
		{
			name: "100000 MWei",
		...
		{
			name: "10000000 ether",
			args: args{value: big.NewFloat(10000000000000000000)},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := NewMWei(tt.args.value); !reflect.DeepEqual(got, tt.want) {
				t.Errorf("NewMWei() = %v, want %v", got, tt.want)
			}
		})
	}
}

func TestUnit_MWei(t *testing.T) {
	type fields struct {
		Value *big.Int
	}
	...
	tests := []struct {
		...
		},
		{
			...
			want:   big.NewFloat(100000000000000),
		},
		{
			name:   "1000 ether",
		...
		},
		{
			...
			want:   big.NewFloat(10000000000000000000),
		},
		{
			name:   "91231929312.891218293912932993 ether",
			fields: fields{Value: decEther},
			want:   big.NewFloat(91231929312891218293912.932993),
		},
		{
			name:   "a lot ether",
			fields: fields{Value: veryBigEther},
			want:   big.NewFloat(99999999999999999899879999999999999999999999999999999999999999999999999999999000000000000),
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got := u.MWei(); got.String() != tt.want.String() {
				t.Errorf("MWei() = %s, want %s", got.String(), tt.want.String())
			}
		})
	}
}
